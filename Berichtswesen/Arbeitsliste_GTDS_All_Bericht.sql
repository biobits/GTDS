...
*/
---Arbeistliste
select Q.Typ,Q.Patienten_ID,q.GTDS_ID,Q.Im_GTDS,Q.EXTERNE_DIAGNOSEN,Q.Schnittstellen_Filter,Q.Diagnose_Tumor,Q.DIAGNOSEDATUM,
EXTRACT(year FROM Q.DIAGNOSEDATUM) as Diagnosejahr
,Q.Arbeittsliste_Datum,Q.Arbeitsliste_Tumor,Q.Arbeitsliste_Bemerkung,/*Q.Behandelt_In,
case when instr(Behandelt_In, '2018')!=0 then 2018 
  when instr(Behandelt_In, '2017')!=0 then 2017
  when instr(Behandelt_In, '2016')!=0 then 2016
  when instr(Behandelt_In, '2015')!=0 then 2015
  else null end as Aktuell_Behandelt_in,*/
  ...
  ( case when (select count (*) from VORHANDENE_DATEN where FK_PATIENTPAT_ID=Q.GTDS_ID 
   ...
   (select qa.AUSPRAEGUNG
    ...
    on qm.ID=qb.FK_QUALITATIVE_FK
          ...
          Q.TUMOR_ID=qb.FK_VORHANDENE_DLFD
          and Q.GTDS_ID=qb.FK_VORHANDENE_DFK
            AND qb.Fk_Qualitative_Fk =83  
            -- Merkmale für Bearbeitet in 83,74,69,55
            AND qb.Fk_Vorhandene_DDAT = 'Diagnose' ) as Behandelt_In_2018, 
Q.MAX_EXT_DAT, 1 as Anzahl
from
(
...
case when p.pat_id is not null then 1 else 0 end Im_GTDS,
  ...
  /*  (select LISTAGG(qm.MERKMAL,' | ') WITHIN GROUP (order by qm.MERKMAL desc)
    FROM QUALITATIVER_BEFUND qb 
    inner join QUALITATIVE_AUSPRAEGUNG qa
    on qb.FK_QUALITATIVE_ID=qa.ID and qb.Fk_Qualitative_Fk=qa.FK_QUALITATIVESID
    inner join Qualitatives_Merkmal qm
    on qm.ID=qb.FK_QUALITATIVE_FK
          ...
          and T.FK_PATIENTPAT_ID=qb.FK_VORHANDENE_DFK
            AND qb.Fk_Qualitative_Fk in (83,74,69,55)  
            and qa.AUSPRAEGUNG='J'-- Merkmale für Bearbeitet in 83,74,69,55
            AND qb.Fk_Vorhandene_DDAT = 'Diagnose' ) as Behandelt_In,*/
...
0 as LINK_MISSING,1 as IM_GTDS,t.ICD10 as EXTERNE_DIAGNOSE,NVL(b.KLASSE_KURZ,'Rest') Schnittstellen_Filter,
...
(select ED.DATUM as Datum,'Diag.' as Typ,ED.FK_EXTERNE_PATIENTEN_ID PATIENTEN_ID,ED.IMPORT_QUELLE  from EXTERNE_DIAGNOSE ED 
  ...
  union all
  select apb.BEGINN,'Aufe.',FK_EXTERNE_PATIENTEN_ID,apb.IMPORT_QUELLE from ABTEILUNG_PATIENT_BEZIEHUNG apb where apb.BEGINN<=SYSDATE ) XT
  where XT.PATIENTEN_ID =pa.PATIENTEN_ID and XT.IMPORT_QUELLE='UKE'
  and XT.DATUm is not null
